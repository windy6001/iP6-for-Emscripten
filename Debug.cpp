...
byte Debug(reg *R)
{
  ...
  while(1)
  {
    ...
    switch(S[0])
    {
      case 'X': ramdump(); break;
      case 'S': UpdateScreen(); break;
      case 'H':
      case '?':
      ...
      case '\0': return(1);
      case '=':  if(strlen(S)>=2)
                 { sscanf(S+1,"%hX",&Trap);Trace=0;return(1); }
                 break;
      case '+':  if(strlen(S)>=2)
                 {
                   sscanf(S+1,"%hX",&Trap);
      ...
      case 'J':  if(strlen(S)>=2)
                 { sscanf(S+1,"%hX",&(R->PC.W));Trace=0;return(1); }
                 break;
      ...
      case 'M':
        {
          ...
          for(J=0;J<16;J++)
          {
            printf("%04X: ",Addr);
            for(I=0;I<16;I++,Addr++)
              printf("%02X ",M_RDMEM(Addr));
            printf(" | ");Addr-=16;
            for(I=0;I<16;I++,Addr++)
              putchar(isprint(M_RDMEM(Addr))? M_RDMEM(Addr):'.');
            puts("");
          }
        }
        break;
