...
ColTyp BPal[16], BPal11[4], BPal12[8], BPal13[8], BPal14[4], BPal15[8],
    BPal53[32];
...
byte *Keys;
byte Keys1[256][2], Keys2[256][2], Keys3[256][2], Keys4[256][2], Keys5[256][2],
    Keys6[256][2], Keys7[256][2];
int scr4col = 0;
static long psec = 0, pusec = 0;

void LockSurface(void) { SDL_LockSurface(surface); }

void UnlockSurface(void) { SDL_UnlockSurface(surface); }
...
int InitMachine(void) {
  ...
  sdlWindow = SDL_CreateWindow("iP6 ", // ウィンドウのタイトル
...
void Keyboard(void) {
  ...
  if (ret == 0)
    return;
  if( e.type == SDL_QUIT) {   // FIX ME  落ちてしまう？
    TrashP6();
    TrashMachine();
    SDL_Quit();
  }
  ...
  if (e.type == SDL_KEYDOWN || e.type == SDL_KEYUP) {
    ...
    if (e.type == SDL_KEYDOWN) {
      ...
      //        case XK_F12: CPURunning=0;break;
              ...
              case SDLK_INSERT: J=SDLK_F13;break; // Ins -> F13
            }
          
      if ((P6Version == 0) && (J == 0xFFC6))
      ...
      if (Keys[1] != 0x00)
        KeyIntFlag = INTFLAG_REQ;
          ...
          if (J==SDLK_LALT) kbFlagGraph=0;
          if (J==SDLK_LCTRL) kbFlagCtrl=0;
    }
  }
...
void InitColor(int screen_num) {
  ...
  int param[16][3] = /* {R,G,B} */
      {{0, 0, 0}, {4, 3, 0}, {0, 4, 3}, {3, 4, 0}, {3, 0, 4}, {4, 0, 3},
       {0, 3, 4}, {3, 3, 3}, {0, 0, 0}, {4, 0, 0}, {0, 4, 0}, {4, 4, 0},
  ...
  for (i = 0; i < 4; i++)
    BPal11[i] = BPal[Pal11[i]];
  for (i = 0; i < 8; i++)
    BPal12[i] = BPal[Pal12[i]];
  for (i = 0; i < 8; i++)
    BPal13[i] = BPal[Pal13[i]];
  for (i = 0; i < 4; i++)
    BPal14[i] = BPal[Pal14[i]];
  for (i = 0; i < 8; i++)
    BPal15[i] = BPal[Pal15[i]];
  for (i = 0; i < 32; i++)
    BPal53[i] = BPal[Pal53[i]];
}
